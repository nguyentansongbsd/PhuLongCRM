...
namespace PhuLongCRM.Views
{
    ...
    public partial class AccountDetailPage : ContentPage
    {
        ...
        private void SetButtonFloatingButton()
        {
            if (viewModel.singleAccount.accountid != Guid.Empty)
            {
                if (viewModel.singleAccount.statecode != "1")
                {
        ...
        private async void DeleteMandatory_Clicked(object sender, EventArgs e)
        {
            ...
            if (IsSuccess)
            {
                ...
                LoadingHelper.Hide();
                ToastMessageHelper.Message(Language.da_xoa_nguoi_uy_quyen_duoc_chon);
            }
            else
            {
                LoadingHelper.Hide();
                ToastMessageHelper.Message(Language.xao_nguoi_uy_quyen_that_bai);
            }
        }
